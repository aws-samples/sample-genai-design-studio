...
**サポートモデル:**
- **Amazon Nova Canvas**（デフォルト）: 詳細なパラメーター制御による高品質な画像生成
- **Amazon Nova 2 Omni**: 次世代マルチモーダルモデルによる高速並列生成

**主要機能:**
- **テキスト入力**: 英語・日本語など様々な言語でプロンプトによる詳細な画像描写指定
- **モデル選択**: `model_id`パラメーターでNova CanvasまたはNova 2 Omniを選択
- **生成パラメータ**:
  - CFGスケール（1.1-10.0）による品質制御 - Nova Canvasのみ
  - 画像サイズ選択（1024x1024, 768x1344等）
  - 複数画像同時生成（最大5枚）
  - Nova 2: 並列実行による高速な複数画像生成
